export default {
    previewDebitNote: async (dnHeader) => {
        try {
            ...
            // --- HELPER: AMOUNT TO WORDS (Original) ---
            const toWords = (num) => {
            ...
            // --- HELPER: DRAW RECURRING HEADER (Original) ---
            const drawMainHeader = (currentPage, totalPages) => {
                ...
                const vArr = [dnHeader.vendor_address, dnHeader.vendor_city, dnHeader.vendor_district, dnHeader.vendor_pin ? "PIN: " + dnHeader.vendor_pin : null].filter(Boolean).join(", ");
            ...
            doc.autoTable({
                ...
                body: dnLines.map((row, index) => {
                    ...
                    return [
                        ...
                        row['Disc %'] + "%", Number(row['Disc. $'] || 0).toFixed(2),
                ...
                theme: 'grid',
                styles: { fontSize: 6, cellPadding: 2, lineColor: [0, 0, 0], lineWidth: 0.5, textColor: [0, 0, 0] },
            ...
            const slipContentY = slipY + 40; doc.setFont("helvetica", "normal");
            ...
            // --- 5. THE FIX: DIRECT DOWNLOAD ---
            const fileName = (dnHeader.debit_note_number || "DebitNote") + ".pdf";
            download(doc.output('dataurlstring'), fileName, "application/pdf");
            showAlert("Debit Note Downloaded Successfully", "success");
    ...
    _drawSimpleBox: (doc, x, y, width, height, rows) => {
        ...
        rows.forEach(r => {
            ...
            doc.setFont("helvetica", "normal");
            const val = String(r[1] || "-"); const splitVal = doc.splitTextToSize(val, width - 62); 
            doc.text(splitVal, x + 58, rowY); rowY += (splitVal.length * 9) + 1.5; 
        });
    }
}
